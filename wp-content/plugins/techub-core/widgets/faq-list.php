...
class Techub_Faq_List extends Widget_Base {
	...
	protected function  register_controls_section(){
		...
		$this->add_control(
			'item_list',
			[
				'label' => esc_html__( 'Service Item List', 'textdomain' ),
				...
				<?php foreach( $settings['item_list'] as $key => $item):
					...
					<div class="tp-faq-5-accordion-item accordion-item mb-10">
						<h2 class="accordion-header">
							<button class="tp-faq-5-accordion-title accordion-button <?php echo esc_attr($collapsed);?> " type="button" 
							data-bs-toggle="collapse" data-bs-target="#flush-collapseOne-<?php echo esc_attr($key);?>" aria-expanded="false" 
							aria-controls="flush-collapseOne-<?php echo esc_attr($key);?>">
							<?php echo esc_html($item['techub_title']) ?>
							<span class="accordion-btn"></span>
							</button>
						</h2>

					<div id="flush-collapseOne-<?php echo esc_attr($key);?>" class="accordion-collapse collapse <?php echo esc_attr($show);?>" 
